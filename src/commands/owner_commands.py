...
class OwnerCommands(commands.Cog):
    ...
    async def admin_commands(self, ctx):  # FIXED: Added self parameter
        ...
        if ctx.invoked_subcommand is None:
            ...
            basic_commands = [
                ("!admin status", "Show detailed bot status"),
                ("!admin restart", "Restart bot service"),
                ("!admin logs [lines]", "Show recent log entries"),
                ("!admin health", "Comprehensive health check")
            ]
            
            advanced_commands = [
                ("!admin update [force]", "Update bot from GitHub"),
                ("!admin backup", "Create manual backup"),
                ("!admin cleanup", "Clean temporary files"),
                ("!admin performance", "Show performance metrics")
            ]
            ...
            embed.add_field(
                ...
                value="\n".join([f"`{cmd}` - {desc}" for cmd, desc in advanced_commands]), 
                inline=False
            )
